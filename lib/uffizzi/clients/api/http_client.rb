...
module Uffizzi
  class HttpClient
    class << self
      ...
      def has_basic_auth_params?
        ConfigFile.exists? && ConfigFile.option_has_value?(:basic_auth_user) && ConfigFile.option_has_value?(:basic_auth_password)
      end
    end
  end
end
